            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200">
              <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{daySchedules.length}건</p>
                </div>
                <div className="flex gap-1.5">
                  ...
                  <button
                    onClick={() => { setSwapMode(!swapMode); setSwapFirstSchedule(null); }}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium ${
                      swapMode ? "bg-orange-500 text-white" : "bg-gray-100 text-gray-600"
                    }`}
                  >
                    교환
                  </button>
              ...
              {daySchedules.length === 0 ? (
                ...
                <div className="divide-y divide-gray-100">
                  {daySchedules.map((s) => {
                    ...
                    const statusLabel = s.status === "confirmed" ? "확정" : s.status === "pending" ? "대기" : "교환요청";
                    const statusClass = s.status === "confirmed" ? "bg-green-100 text-green-700" : s.status === "pending" ? "bg-yellow-100 text-yellow-700" : "bg-orange-100 text-orange-700";

                    return (
                      <div
                        key={s.id}
                        className="px-4 py-3 active:bg-gray-50 cursor-pointer"
                        onClick={() => swapMode ? handleSwapSelect(s) : setDetailSchedule(s)}
                      >
                        {/* 헤더: 제목 + 상태 + 버튼 */}
                        <div className="flex items-center gap-2 mb-1">
                          <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: color }} />
                          <span className="text-sm font-bold text-gray-800 flex-1 truncate">{s.title}</span>
                          <span className={`text-[10px] px-1.5 py-0.5 rounded-full shrink-0 ${statusClass}`}>{statusLabel}</span>
                          {!swapMode && (
                            <div className="flex gap-0.5 shrink-0">
                              {isAdmin && s.status !== "unassigned" && (
                                <button onClick={(e) => { e.stopPropagation(); handleUnassignSchedule(s.id); }} className="p-1 text-gray-400 active:text-orange-500 rounded" title="반환">
                                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                                </button>
                              )}
                              <button onClick={(e) => { e.stopPropagation(); handleEditSchedule(s); }} className="p-1 text-gray-400 active:text-blue-500 rounded">
                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                              </button>
                              <button onClick={(e) => { e.stopPropagation(); handleDeleteSchedule(s.id); }} className="p-1 text-gray-400 active:text-red-500 rounded">
                        ...
                        {/* 시간/담당 */}
                        <div className="text-xs text-gray-500 mb-1.5 ml-4">
                          {s.memberName}
                        </div>
                        {/* 본문 (예약양식) 전체 표시 */}
                        {s.note && (
                          <div className="ml-4 p-2.5 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-700 whitespace-pre-wrap leading-relaxed">
                            {s.note}
                          </div>
                        )}
                      </div>
