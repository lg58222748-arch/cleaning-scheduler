...
export async function GET(req: NextRequest) {
  ...
  return Response.json(getCommentsBySchedule(scheduleId));
}
...
export async function POST(req: NextRequest) {
  ...
  if (body.action === "delete" && body.id) {
    const deleted = deleteComment(body.id);
  ...
  const comment = addComment(body.scheduleId, body.authorName, body.content);
